...
def parse_args():
    ...
    parser.add_argument('--img_prefix', help='img path')
    parser.add_argument('--test_scale', help='test_scale')
    parser.add_argument(
        '--format_only',
        action='store_true',
...
def main():
    ...
    if args.json is not None:
        cfg.data.test.ann_file = args.json
    if args.img_prefix is not None:
        cfg.data.test.img_prefix = args.img_prefix
    if args.test_scale is not None:
        scale_arr = args.test_scale.split(",")
        scale = (int(scale_arr[0]), int(scale_arr[1]))
